...
class LhostYmlFileChangerForMultipleSites():
	...
	self.keyName3 = 'username'
	self.keyName4 = 'password'
    ...
    def change_values_for_all_sites(self, site_list, new_value_for_keyname1, new_value_for_keyname2):
        for site in site_list:
            try:
                ...
                log.error('while updating the site {site} error occurred:\n {e}'.format(site = site, e =e))
    ...
    def change_values_for_one_site(self, site, new_value_for_keyname1, new_value_for_keyname2):
        ...
        with temporary.temp_dir(parent_dir='/tmp/') as tmp_work_dir:
            ...
            with open(teml_yml_fl) as scanner_conts:
                ...
                lhost_yml_dict['shinken_resources'][self.keyName2] = QuotedString(self.encrypt(site.upper() + new_value_for_keyname2))
                lhost_yml_dict['endpoints'] = lhost_yml_dict.get('endpoints', {})
                length_list = len(lhost_yml_dict['endpoints'])
		# Updating the username and password $user$ from lowercase to uppercase
                for index_usr in range(length_list):
                    if type(lhost_yml_dict['endpoints'][index_usr][self.keyName3]) is dict:
		        orig_usr = lhost_yml_dict['endpoints'][index_usr][self.keyName3]['$resource']
                        lhost_yml_dict['endpoints'][index_usr][self.keyName3]['$resource'] = re.sub('(\$user)+', '$USER', orig_usr, flags=re.I)
		    if type(lhost_yml_dict['endpoints'][index_usr][self.keyName4]) is dict:
		        orig_pwd = lhost_yml_dict['endpoints'][index_usr][self.keyName4]['$resource']
                        lhost_yml_dict['endpoints'][index_usr][self.keyName4]['$resource'] = re.sub('(\$user)+', '$USER', orig_pwd, flags=re.I)
            self.process_config_file('/lhost.yml', yaml.dump(lhost_yml_dict, default_flow_style=False, Dumper = MyDumper))
            self.log_status()
